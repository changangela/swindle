...
-- eval env (List ((Atom "map") : rest)) = _map eval rest
...
primitiveBindings = nullEnv >>= (flip bindVars $ map (bindFunc IOFunc) ioPrimitives ++ map (bindFunc PrimitiveFunc) primitives)
...
swindleEnv :: IO Env
swindleEnv = primitiveBindings >>= (flip bindVars [("empty", List [])])
